...
Settings now lives at `/settings`, with its own full-page shell and section navigation, without the workspace sidebar/chat panels. Agent jobs reference their triggering message via migration `000010`; turn input includes JSON participant IDs/names, exact sender, and up to 20 prior human messages (2000 characters each, truncation marked). This context is assembled at dispatch, excludes profile emails/avatars, and is explicitly treated as untrusted data. Legacy jobs keep unknown sender attribution rather than guessing by prompt text.
